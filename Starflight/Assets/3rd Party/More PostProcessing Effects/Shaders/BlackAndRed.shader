...
Shader "MorePPEffects/BlackAndRed" {
	Properties {
	...
	float4 frag(v2f i) : SV_Target 
	{
        ...
        float threshold = smoothstep(0.0f, smoothness, inCol.r - grayscale);
        grayscale = pow(grayscale * 1.1f, 2.0f);
        outCol = lerp(grayscale, inCol * float3(1.1f, 0.5f, 0.5f), threshold);
