...
public class HydraulicErosion
{
	...
	public float[,] Process( float xyScaleToMeters, float zScaleToMeters, float rainWaterAmount, float sedimentCapacity, float gravityConstant, float frictionConstant, float evaporationConstant, float depositionConstant, float dissolvingConstant, float stepDeltaTime, int snapshotInterval, int finalBlurRadius )
	{
		...
		// allocate color buffer (for saving png images)
		m_colorBuffer = new Color[ m_height, m_width ];
	...
	void SaveAsPNG( int iteration )
	{
		...
		for ( var y = 0; y < m_height; y++ )
		{
			for ( var x = 0; x < m_width; x++ )
			{
				...
				if ( delta != 0.0f )
				{
					m_colorBuffer[ y, x ] = new Color( originalHeight - delta, originalHeight + delta, originalHeight - Mathf.Abs( delta ) );
				}
				else
				{
					m_colorBuffer[ y, x ] = new Color( originalHeight, originalHeight, originalHeight );
				}
			}
		}

		Tools.SaveAsPNG( m_colorBuffer, "Erosion Iteration " + iteration );
	}
	...
	class Drop
	{
		...
		public bool Update( int iteration )
		{
